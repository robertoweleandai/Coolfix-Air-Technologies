...
// Nairobi HQ Coordinates
const HQ_POSITION: [number, number] = [-1.2598, 36.8041]; // Westlands area approx
...
const App: React.FC = () => {
  ...
  // Use Intersection Observer to highlight current section in navigation
  useEffect(() => {
    ...
    const sectionIds = ['home', 'residential', 'networking', 'consulting', 'contact'];
  ...
  const renderContact = () => (
    <section id="contact" className="pt-44 pb-32 container mx-auto px-6">
      ...
      <div className="grid lg:grid-cols-3 gap-10 items-stretch">
        ...
        {/* Right Column: Deploy Request Form (Spans 2 columns on desktop) */}
          ...
          <form className="grid md:grid-cols-2 gap-10" onSubmit={(e) => { e.preventDefault(); handleWhatsAppRedirect("Automated site audit request initiated from web portal."); }}>
            ...
            <div className="space-y-8 flex flex-col">
              <div className="space-y-3 flex-1">
                ...
                <textarea placeholder="Describe your technical requirements..." className="w-full h-full bg-slate-950/50 border border-white/10 rounded-3xl p-6 outline-none font-bold focus:border-cyan-500 transition-all text-white resize-none shadow-inner min-h-[200px]"></textarea>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30 overflow-x-hidden">
      {/* Precision Navigation - No Logo, Centralized Links */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-24 mb-32 text-center md:text-left">
            ...
            <div>
              ...
              <ul className="space-y-6">
                {['Fiber Tiers', 'Hotspot Node', 'Infrastructure', 'Site Audit'].map(l => (
                  <li key={l}>
                    <button 
                      onClick={() => {
                        const target = (l === 'Fiber Tiers' || l === 'Hotspot Node') ? 'residential' : l === 'Infrastructure' ? 'networking' : 'contact';
