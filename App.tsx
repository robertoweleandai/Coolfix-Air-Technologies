...
const NetworkAlertBanner = () => {
  const alerts = [
    "BACKBONE UPGRADE: Sector Delta nodes may experience 15ms latency spikes during 02:00-04:00 UTC.",
    "MAINTENANCE: Fiber Splicing active in Westlands Sector. Uplink redirection in progress.",
    "NETWORK STATUS: All Nairobi HQ Centipid Nodes are OPERATIONAL. Shield Layer active.",
    "GLOBAL UPLINK: Redundant satellite failover initialized for Parklands Node 04."
  ];

  return (
    <div className="bg-cyan-950/40 border-b border-cyan-500/20 h-10 flex items-center overflow-hidden relative z-[70] backdrop-blur-md">
      <div className="absolute left-0 h-full bg-cyan-600 px-4 flex items-center gap-3 z-10 shadow-[20px_0_40px_rgba(0,0,0,0.5)]">
        <i className="fa-solid fa-triangle-exclamation text-white text-[10px] animate-pulse"></i>
        <span className="text-[10px] font-black text-white uppercase tracking-widest">System Alert</span>
      </div>
      <div className="flex whitespace-nowrap animate-marquee">
        {[...alerts, ...alerts].map((alert, i) => (
          <span key={i} className="mx-12 text-[10px] font-bold text-cyan-200/80 uppercase tracking-[0.2em] flex items-center gap-4">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30 overflow-x-hidden flex flex-col">
      ...
      <nav className={`fixed top-10 left-0 right-0 z-[60] transition-all duration-700 ${isScrolled ? 'bg-slate-950/90 backdrop-blur-3xl py-5 border-b border-white/5 shadow-2xl translate-y-[-40px]' : 'bg-transparent py-10'}`}>
        ...
        <section id="tracking" className="pt-40 pb-32 container mx-auto px-6">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="bg-slate-900/60 border border-white/5 p-12 rounded-[4rem] shadow-3xl relative overflow-hidden">
              ...
              {trackResult && (
                <div className="animate-in fade-in space-y-16 mt-16 pb-8">
                  <div className="relative pt-10 max-w-2xl mx-auto">
                    <div className="absolute top-[40%] left-0 w-full h-1 bg-slate-800 rounded-full"></div>
                    <div 
                      className="absolute top-[40%] left-0 h-1 bg-emerald-500 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(16,185,129,0.5)]"
                      style={{ width: `${Math.min((displayStage / 3) * 100, 100)}%` }}
                    ></div>
                    <div className="relative flex justify-between">
                      {[1, 2, 3].map((s) => (
                        <div key={s} className="flex flex-col items-center gap-5">
                          <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-700 z-10 border-4 ${
                            displayStage >= s 
                            ? 'bg-emerald-500 border-slate-900 text-slate-950 shadow-[0_0_20px_rgba(16,185,129,0.5)]' 
                            : 'bg-slate-900 border-slate-800 text-slate-600'
                          }`}>
                            <i className={`fa-solid ${s === 1 ? 'fa-clipboard-check' : s === 2 ? 'fa-truck-fast' : 'fa-signal'} text-lg`}></i>
                          </div>
                          <div className="text-center">
                            <p className={`text-[9px] font-black uppercase tracking-[0.2em] transition-colors duration-700 ${displayStage >= s ? 'text-emerald-500' : 'text-slate-600'}`}>
                              {s === 1 ? 'Received' : s === 2 ? 'Field Tech' : 'Active Node'}
                            </p>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="flex flex-col items-center text-center space-y-6">
                    <div className={`px-10 py-3.5 rounded-full text-[11px] font-black uppercase tracking-[0.4em] shadow-2xl border transition-all duration-700 ${
                      displayStage >= trackResult.stage 
                      ? 'bg-emerald-600/10 border-emerald-500/30 text-emerald-500' 
                      : 'bg-slate-900 border-white/5 text-slate-600'
                    }`}>
                      {displayStage >= trackResult.stage ? trackResult.status : 'Analyzing Infrastructure...'}
                    </div>
                    <div className="h-16 flex items-center justify-center">
                       <h4 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tighter leading-none animate-in slide-in-from-bottom-2 duration-500">
                        {displayStage >= trackResult.stage ? trackResult.message : ''}
                       </h4>
                    </div>
                  </div>
                </div>
