...
import { FIBER_PACKAGES, NETWORKING_SERVICES, CONSULTING_SERVICES, MOCK_USAGE_DATA } from './constants';
...
const App: React.FC = () => {
  ...
  const renderHome = () => (
    <div className="space-y-32 pb-32">
      ...
      {/* Core Protocols Grid - Replaces Chart Section */}
  ...
  const renderResidential = () => (
    <div className="pt-40 pb-32 container mx-auto px-6">
      <div className="mb-24 text-center">
        <h2 className="text-6xl font-black text-white mb-8 tracking-tighter uppercase leading-none">Access <span className="text-cyan-500">Tiers.</span></h2>
        <p className="text-xl text-slate-400 font-medium max-w-2xl mx-auto leading-relaxed">Select your bandwidth node. All plans include the Shield protection layer and Westlands backbone routing.</p>
      </div>
          ...
          <div key={pkg.id} className={`relative p-10 rounded-[3.5rem] bg-slate-900/60 border transition-all duration-700 flex flex-col h-full group ${pkg.isPopular ? 'border-cyan-500 shadow-[0_0_80px_-20px_rgba(6,182,212,0.4)] scale-105 z-10' : 'border-white/5 hover:border-white/10'}`}>
            ...
            <div className="mb-10 flex justify-between items-start">
              <div>
                ...
                <p className="text-4xl font-black text-white tracking-tighter">{pkg.speed}</p>
              </div>
            ...
            <div className="mb-10 flex items-baseline">
              ...
              <span className="text-slate-500 text-[11px] ml-3 font-black uppercase tracking-widest">/ Mo</span>
            </div>
  ...
  const renderContact = () => (
    <div className="pt-44 pb-32 container mx-auto px-6">
      <div className="grid lg:grid-cols-2 gap-24 items-start">
        <div className="space-y-16">
          <div>
            ...
            <p className="text-2xl text-slate-400 font-medium leading-relaxed max-w-md">Our engineering teams are ready to deploy your next high-speed environment.</p>
          </div>

          <div className="space-y-8">
            <div className="group bg-slate-900/60 border border-white/5 p-10 rounded-[4rem] hover:border-cyan-500/30 transition-all duration-700 flex items-center gap-10 shadow-3xl">
            ...
            <div 
              className="group bg-slate-900/60 border border-white/5 p-10 rounded-[4rem] hover:border-emerald-500/30 transition-all duration-700 flex items-center gap-10 cursor-pointer shadow-3xl" 
            ...
            <div 
              className="group bg-slate-900/60 border border-white/5 p-10 rounded-[4rem] hover:border-blue-500/30 transition-all duration-700 flex items-center gap-10 cursor-pointer shadow-3xl" 
              onClick={() => window.open(MAPS_URL, '_blank')}
            >
              <div className="w-20 h-20 bg-blue-600/10 rounded-3xl flex items-center justify-center text-blue-400 text-3xl group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner">
                <i className="fa-solid fa-map-location-dot"></i>
              </div>
              <div>
                <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-2">Satellite HQ</p>
                <p className="text-4xl font-black text-white group-hover:text-blue-400 transition-colors tracking-tighter uppercase">Westlands, Nairobi</p>
              </div>
            </div>
          </div>
        </div>

        <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/10 p-16 rounded-[5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-black text-white mb-10 tracking-tight">Deploy Request</h3>
          <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); handleWhatsAppRedirect("Automated site audit request initiated from web portal."); }}>
            ...
            <div className="space-y-3">
              ...
              <textarea placeholder="Describe your technical requirements..." rows={5} className="w-full bg-slate-950/50 border border-white/10 rounded-3xl p-6 outline-none font-bold focus:border-cyan-500 transition-all text-white resize-none shadow-inner"></textarea>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30 overflow-x-hidden">
      {/* Precision Navigation - Logo Removed */}
          ...
          <div className="flex items-center space-x-14">
            {[
              ...
              <button 
                ...
                className={`text-[10px] font-black uppercase tracking-[0.4em] transition-all hover:text-cyan-400 relative py-2 ${view === link.id ? 'text-cyan-400' : 'text-slate-500'}`}
              >
                {link.label}
                {view === link.id && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-cyan-500 rounded-full animate-in zoom-in duration-500"></span>}
              </button>
            ))}
            <a 
              href={PORTAL_URL} 
              ...
              className="px-8 py-3.5 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] transition-all backdrop-blur-md shadow-inner"
            >
              Control Hub
            </a>
          </div>
          
          <button className="md:hidden absolute right-6 text-white text-2xl w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10" onClick={() => setIsMenuOpen(!isMenuOpen)}>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[55] bg-slate-950/98 backdrop-blur-3xl p-10 pt-40 flex flex-col space-y-10 animate-in slide-in-from-right duration-500">
          {['home', 'residential', 'networking', 'consulting', 'contact'].map(v => (
            <button key={v} onClick={() => { setView(v as AppView); setIsMenuOpen(false); }} className={`text-5xl font-black uppercase tracking-tighter text-left ${view === v ? 'text-cyan-500' : 'text-white/60'}`}>{v}</button>
          ))}
      ...
      <main className="animate-in fade-in duration-1000">
        ...
        {view === 'residential' && renderResidential()}
        {view === 'networking' && renderNetworking()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-24 mb-32 text-center md:text-left">
            ...
            <div>
              ...
              <ul className="space-y-6">
                {['Fiber Tiers', 'Infrastructure', 'Consulting', 'Site Audit'].map(l => (
                  <li key={l}><button onClick={() => setView(l.toLowerCase().includes('tier') ? 'residential' : l.toLowerCase() as any)} className="text-sm font-bold text-slate-500 hover:text-cyan-400 transition-colors uppercase tracking-widest">{l}</button></li>
                ))}
              </ul>
            </div>
      ...
      <AirAssistant externalQuery={aiAssistantQuery} onQueryHandled={() => setAiAssistantQuery(null)} />
    </div>
  );
};
